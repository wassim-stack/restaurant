...
export const Food = () => {
  ...
  const filterType = (foodType)=>{
    setFoods(
      data.filter((item)=>{return item.category===foodType})
    ...
    <div className='max-w-[1640px] m-auto px-4 py-12'>
       ...
       <div>
            ...
            <div className='flex justify-between flex-wrap'>
                ...
                <button onClick={()=>filterType('chicken')} className='m-1  border-orange-600 text-orange-600 hover:bg-orange-600 hover:text-white'>Chiken</button>
            </div>
        </div>
        {/* Filter Price */}
        <div>
       ...
       <div className='grid grid-cols-2 lg:grid-cols-4 gap-6 pt-4'>
          {foods.map((item,index)=>(
            <div key={item.id} className='border shadow-lg rounded-xl hover:scale-105 duration-300'>
              <img src={item.image} alt={item.name} className="w-full h-[200px] object-cover rounded-t-xl" />
              <div className='flex justify-between rounded-full px-2 py-4'>
